...
/// Pkcs12Store loading extension
public extension Pkcs12Store {
    static func load(data content: Data, password: String) throws -> Pkcs12Store {
    ...
    static func load(atPath path: String, password: String) throws -> Pkcs12Store {
        guard let content = FileManager.default.contents(atPath: path) else {
            throw CocoaError(.coderReadCorrupt)
        }

        return try load(data: content, password: password)
    }

    static func load(at url: URL, password: String) throws -> Pkcs12Store {
